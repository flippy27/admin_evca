  ...
  login: async (email: string, password: string, rememberMe: boolean) => {
    try {
      ...
      try {
        ...
        console.warn(
          '[Auth] Failed to fetch permissions, using JWT claims:',
          permError,
        )
  ...
  restoreSession: async () => {
    try {
      ...
      const refreshTokenExpiry =
        (snapshot.timestamp || 0) + (snapshot.refreshExpiresIn || 0) * 1000
      ...
      const accessTokenExpiry =
        (snapshot.timestamp || 0) + (snapshot.expiresIn || 0) * 1000
      ...
      try {
        ...
        const updatedUser: ProcessedUserData = {
          userId:
            jwtClaims.userId ||
            currentUser?.userId ||
            permissions.user.externalId,
          email:
            jwtClaims.email || currentUser?.email || permissions.user.email,
        ...
        logger.warn(
          'Failed to refresh permissions, but token refresh succeeded',
          permError,
        )
