...
export function OperadorChargerDetail({ charger }: { charger: any }) {
  ...
  const executeCommand = async (command: string, connectorId?: number) => {
    const key = connectorId ? `${command}-${connectorId}` : command;
        ...
        {charger.connectors?.map((connector: any) => {
          ...
          return (
            ...
            >
              ...
              {/* Vehicle & charging info (all roles see this) */}
              {connector.vehicleId && (
              ...
              <View style={{ gap: 8 }}>
                ...
                <View style={{ flexDirection: "row", gap: 8 }}>
                  {!charging ? (
                    <TouchableOpacity
                      onPress={() => executeCommand("start", connector.connectorId)}
                    ...
                    >
                      {actionLoading === `start-${connector.connectorId}` ? <ActivityIndicator size="small" color="white" /> : <Ionicons name="play" size={15} color="white" />}
                      <Text style={{ fontSize: 13, fontWeight: "600", color: "white" }}>Iniciar Carga</Text>
                    </TouchableOpacity>
                  ) : (
                    <TouchableOpacity
                      onPress={() => executeCommand("stop", connector.connectorId)}
                    ...
                    >
                      {actionLoading === `stop-${connector.connectorId}` ? <ActivityIndicator size="small" color="white" /> : <Ionicons name="stop" size={15} color="white" />}
                      <Text style={{ fontSize: 13, fontWeight: "600", color: "white" }}>Detener Carga</Text>
                    </TouchableOpacity>
                  )}
                  <TouchableOpacity
                    onPress={() => executeCommand("unlock", connector.connectorId)}
                  ...
                  >
                    {actionLoading === `unlock-${connector.connectorId}` ? <ActivityIndicator size="small" color="white" /> : <Ionicons name="lock-open" size={15} color="white" />}
